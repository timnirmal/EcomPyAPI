...
def userdata_all():
    ProfileDataList = supabase.table('profiles').select('*').execute()

    print(ProfileDataList)

    return ProfileDataList


def userdata(person_id):
    ProfileDataList = supabase.table('profiles').select('*').eq('id', person_id).execute()

    print(ProfileDataList)

    return ProfileDataList

# return search(name, brand, priceMin, priceMax, rating, color, gender)
def search(name, brand, priceMin, priceMax, rating, color, gender, category):
    ...
    df = df[df['price'].str[1:].apply(lambda x: x.replace(',', '')).apply(lambda x: float(x)) >= priceMin]
    df = df[df['price'].str[1:].apply(lambda x: x.replace(',', '')).apply(lambda x: float(x)) <= priceMax]
    df = df[df['rating'].apply(lambda x: float(x)) >= rating]
    df = df[df['PrimaryColor'].str.contains(color, case=False, na=False)]
